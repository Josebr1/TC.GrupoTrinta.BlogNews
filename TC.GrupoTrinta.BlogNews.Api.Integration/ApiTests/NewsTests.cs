...
public class NewsTests : IClassFixture<WebApplicationFactory<Program>> {
  ...
  public NewsTests(WebApplicationFactory<Program> testFixtureBase) {
  ...
  public async Task CreateNews_WorkAsExpected() {
    var payload =
        new CreateNewsInput() { Title = "Title", Description = "Description",
                                Author = "Author",
  ...
  public async Task GetNews_WorkAsExpected() {
    ...
    var result =
        JsonConvert.DeserializeObject<List<NewsModelOutput>>(jsonString);
